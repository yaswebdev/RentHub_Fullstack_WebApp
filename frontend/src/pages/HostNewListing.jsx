...
import { creerPropriete } from '../api/proprietesAPI';
import { uploadPhotos } from '../api/photosAPI';
import { useToast } from '../context/ToastContext';

export const HostNewListing = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const created = await creerPropriete(payload);
      const annonceId = created?.id;
      if (annonceId && photos.length > 0) {
        try {
          const batchSize = 2;
          for (let i = 0; i < photos.length; i += batchSize) {
            const batch = photos.slice(i, i + batchSize);
            await uploadPhotos(annonceId, batch);
          }
        } catch (uploadError) {
          toast('Annonce créée, mais l\'upload des photos a échoué.', 'warning');
          navigate('/dashboard');
          return;
        }
      }
      toast('Annonce créée avec succès.', 'success');
      navigate('/dashboard');
    } catch (err) {
      toast(err.message || 'Impossible de créer l\'annonce.', 'error');
    } finally {
      setChargement(false);
    }
  };
        ...
        <Card className="shadow-lg">
          <CardHeader className="border-b border-slate-100">
            ...
            <p className="text-sm text-slate-500">
              Renseignez les informations essentielles pour publier votre logement.
            </p>
          </CardHeader>
          <CardContent className="pt-6">
            <form onSubmit={handleSubmit} className="space-y-6">
              <Input
                label="Titre"
                placeholder="Ex: Riad moderne au cœur de la Médina"
              ...
              <div className="w-full space-y-2">
                ...
                <textarea
                  ...
                  placeholder="Décrivez votre logement, ses points forts, et l'expérience proposée."
              ...
              <div className="w-full space-y-2">
                <label className="text-sm font-bold text-slate-700 tracking-tight">Photos</label>
                  ...
                  <label className="flex items-center gap-3 cursor-pointer text-sm font-semibold text-slate-600">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (rejected > 0) {
                          toast(
                            'Certaines photos sont trop lourdes (5MB max) ou au mauvais format.',
                            'warning'
                          );
                        }
                        setPhotos(valid);
              ...
              <div className="flex items-center justify-between">
                ...
                <Button type="submit" isLoading={chargement}>
                  <Save className="h-4 w-4" /> Publier l'annonce
                </Button>
              </div>
            </form>
          </CardContent>
